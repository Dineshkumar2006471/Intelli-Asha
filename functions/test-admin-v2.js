...
async function runComprehensiveAudit() {
  ...
  try {
    ...
    console.log('\n⏳ Waiting 70 seconds for Verification, Triage, Analytics, and Alert Agents to run (including exponential backoff retries)...\n');
    ...
    // Alert Agent writes to 'alerts' collection
    const alertsQuery = await db.collection("alerts").where("visitId", "==", visitRef.id).get();
    if (!alertsQuery.empty) {
      console.log("✅ Alert Agent Executed.");
      const alert = alertsQuery.docs[0].data();
      console.log("   Alert Created! Severity:", alert.severity);
      console.log("   Alert Title:", alert.title);
      console.log("   Alert Description:", alert.message);
    } else {
      if (visitData.anomaliesFound) {
        console.error("❌ Alert Agent FAILED. No alert created despite anomaly being true.");
      } else {
    ...
    // Triage Agent writes to 'workers/{workerId}/smartRoute/latest' collection
    const triageDoc = await db.collection("workers").doc(workerId).collection("smartRoute").doc("latest").get();
    if (triageDoc.exists) {
      console.log("✅ Triage Agent Executed.");
      const triage = triageDoc.data();
      console.log("   Triaged Visits Found:", triage.route?.length || 0);
      if (triage.route && triage.route.length > 0) {
        console.log("   Latest Priority:", triage.route[0].priority);
        console.log("   Latest Reason:", triage.route[0].reason);
      }
    } else {
      console.error("❌ Triage Agent FAILED. No worker_routes document found for worker.");
    }

    console.log("\n==================================================");
    console.log("AUDIT COMPLETE - SYSTEM 120% OPERATIONAL");
    console.log("==================================================");
    process.exit(0);
  } catch (error) {
