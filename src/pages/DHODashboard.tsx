...
import { onVisitsSnapshot, onAnalyticsSnapshot } from '../services/db';
...
const DHODashboard = () => {
  ...
  useEffect(() => {
    const unsubVisits = onVisitsSnapshot((visits) => {
      ...
      // We no longer set fake AI Brief or PHC data here.
      // That is now handled by the real Analytics Agent listener below.
  ...
  useEffect(() => {
    if (geoLoading || hasManuallyOverridden) return;
    
    // Explicitly set the detected location to the standardized district name
    if (!isEditingLocation) {
      setLocationName(detectedDistrict || 'Your District');
    }
  }, [geoLoading, detectedDistrict, isEditingLocation, hasManuallyOverridden]);

  useEffect(() => {
    // Listen to the real Analytics document generated by the backend Gemini Agent!
    // We listen to 'global' to ensure the dashboard always updates instantly regardless 
    // of minor string mismatches in geolocation APIs across different devices.
    const unsubAnalytics = onAnalyticsSnapshot('global', (data) => {
      if (data) {
        if (data._status === 'error') {
          setAiBrief({
            anomaly: "Analytics generation failed — retrying on next visit",
            recommendation: `Error details: ${data._lastError || 'Unknown error'}`,
            alert: "System Error"
          });
          setPhcs([]);
        } else {
          ...
          if (data.metrics) {
            setMetrics((prev) => ({
              ...prev,
              disbursement_ready: data.metrics.disbursement_ready ?? prev.disbursement_ready,
              data_quality_score: data.metrics.data_quality_score ?? prev.data_quality_score,
            }));
          }
        }
      } else {
  ...
  }, [locationName, geoLoading, detectedDistrict, isEditingLocation]);
                    ...
                    {phcs.length > 0 ? phcs.map((phc, i) => (
                      ...
                      <tr>
                        <td colSpan={6} className="p-8 text-center text-on-surface-variant">
                          {aiBrief && aiBrief.anomaly.includes('No analytics') ? 'No PHC data available for this district. Waiting for visits.' : 'Loading localized PHC data...'}
                        </td>
                      </tr>
                    )}
                  </tbody>
