...
const DHODashboard = () => {
  ...
  useEffect(() => {
    const unsubVisits = onVisitsSnapshot((visits) => {
      ...
      // CRITICAL FIX: Automatically track the district where the field workers are actually working,
      // instead of using the supervisor's local browser geolocation (which causes mismatches).
      if (visits.length > 0 && !isEditingLocation) {
        const latestVisit = visits[0];
        if (latestVisit) {
          const activeDistrict = latestVisit.districtName || latestVisit.locationName || 'Unknown District';
          if (locationName !== activeDistrict) {
            setLocationName(activeDistrict);
          }
        }
      }
  ...
  useEffect(() => {
    // CRITICAL FIX: Instead of calling the full manual generation, we listen
    // to the real Analytics document generated by the backend Gemini Agent!
    const unsubAnalytics = onAnalyticsSnapshot(locationName, (data) => {
      if (data) {
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="font-body-base text-body-base text-on-surface divide-y divide-border-default">
                    {phcs.length > 0 ? phcs.map((phc, i) => (
                      ...
                      <tr>
                        <td colSpan={6} className={`p-8 text-center text-on-surface-variant ${aiBrief && !aiBrief.anomaly.includes('No analytics') ? '' : 'animate-pulse'}`}>
                          {aiBrief && !aiBrief.anomaly.includes('No analytics') ? 'No PHC data available for this district.' : 'Loading localized PHC data...'}
                        </td>
                      </tr>
                    )}
                  </tbody>
